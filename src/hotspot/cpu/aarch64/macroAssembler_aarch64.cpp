...
void MacroAssembler::lookup_virtual_method(Register recv_klass,
  ...
  if (vtable_index.is_register()) {
    ...
    ldr(method_result, Address(method_result, vtable_offset_in_bytes), rscratch1);
  } else {
...
bool MacroAssembler::try_merge_ldst(Register rt, const Address &adr, size_t size_in_bytes, bool is_store) {
  if (ldst_can_merge(rt, adr, size_in_bytes, is_store)) {
    ...
    if (adr.getMode() == Address::base_plus_offset_safe && adr.getMode() == Address::base_plus_offset_unsafe &&
...
void MacroAssembler::ldr(Register Rx, const Address &adr, Register tmp) {
  ...
  if (!try_merge_ldst(Rx, adr, 8, false)) {
    ...
    if (adr.getMode() == Address::base_plus_offset_unsafe) {
      if (adr.base() == Rx) {
        // If dest and base are equal we may need tmp
        assert(tmp != dummy_reg, "tmp reg may be required for offset");
      } else {
        // Tmp can be dest
        tmp = Rx;
      }
...
bool MacroAssembler::ldst_can_merge(Register rt,
  ...
  if ((adr.getMode() != Address::base_plus_offset_safe && adr.getMode() != Address::base_plus_offset_unsafe) || prev != last ) {
    return false;
  }
...
void MacroAssembler::cache_wb(Address line) {
  assert(line.getMode() == Address::base_plus_offset_safe || line.getMode() == Address::base_plus_offset_unsafe, "mode should be base_plus_offset");
