...
void MacroAssembler::lookup_virtual_method(Register recv_klass,
  ...
  if (vtable_index.is_register()) {
    ...
    ldr(method_result, Address(method_result, vtable_offset_in_bytes, true));
  } else {
...
void MacroAssembler::cmpptr(Register src1, Address src2) {
  ...
  ldr(rscratch1, Address(rscratch1, offset, true));
  cmp(src1, rscratch1);
}
...
void MacroAssembler::load_method_holder_cld(Register rresult, Register rmethod) {
  ...
  ldr(rresult, Address(rresult, InstanceKlass::class_loader_data_offset(), true));
}

void MacroAssembler::load_method_holder(Register holder, Register method) {
  ldr(holder, Address(method, Method::const_offset(), true));                      // ConstMethod*
  ldr(holder, Address(holder, ConstMethod::constants_offset(), true));             // ConstantPool*
  ldr(holder, Address(holder, ConstantPool::pool_holder_offset(), true));          // InstanceKlass*
}
...
void MacroAssembler::load_mirror(Register dst, Register method, Register tmp1, Register tmp2) {
  ...
  ldr(dst, Address(rmethod, Method::const_offset(), true));
  ldr(dst, Address(dst, ConstMethod::constants_offset(), true));
  ldr(dst, Address(dst, ConstantPool::pool_holder_offset(), true));
  ldr(dst, Address(dst, mirror_offset, true));
